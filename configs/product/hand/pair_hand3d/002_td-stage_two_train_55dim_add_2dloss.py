...
model = dict(
    type='TopdownPoseLiftingEstimator',
    ...
    lifting_loss=dict(
        type='MultipleLossWrapper',
        losses=[
            dict(type='MSELoss'),  # 3d kpts
            dict(type='MSELoss', loss_weight=1e-5),  # 2d reprojection left
            dict(type='MSELoss', loss_weight=1e-5),  # 2d reprojection right
        ]),
    test_cfg=dict(
        flip_test=False,
        shift_coords=False,
