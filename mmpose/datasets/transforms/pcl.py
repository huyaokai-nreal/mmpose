...
def perspective_grid(P_virt2orig,
                     image_pixel_size,
                     crop_pixel_size_wh,
                     transform_to_pytorch=False):
    batch_size = P_virt2orig.shape[0]

    # create a grid of linearly increasing indices (one for each pixel,
    # going from 0..1)
    ...
    # rs, cs = torch.meshgrid([xs, ys])  # for pytorch >0.4 instead of
    # following two lines
    ...
    grid = pv.unsqueeze(0).expand(
        [batch_size, 3, crop_pixel_size_wh[0], crop_pixel_size_wh[1]])
    ...
    eps = 0.00000001
    bpv_lin_orig_p = bpv_lin_orig[:, :2, :] / (
        eps + bpv_lin_orig[:, 2:3, :])  # projection, divide homogeneous coord

    # go back from linear to two–dimensional outline of points
    bpv_orig = bpv_lin_orig_p.view(batch_size, 2, crop_pixel_size_wh[0],
                                   crop_pixel_size_wh[1])

    # the sampling function assumes the position information on the
    # last dimension
    bpv_orig = bpv_orig.permute([0, 3, 2, 1])

    # the transformed points will be in pixel coordinates ranging from 0 up
    # to the image width/height (unmapped from the original intrinsics matrix)
    # but the pytorch grid_sample function assumes it in -1,..,1;
    # the direction is already correct (assuming negative y axis, which is also
    # assumed by bytorch)
...
def pcl_transforms(bbox_pos_img,
                   bbox_size_img,
                   K,
    ...
    R_virt2orig = virtual_camera_rotation_from_position(p_position)
...
def pcl_transforms_2d(pose2d,
    ...
    P_virt2orig, R_virt2orig, _, _, P_orig2virt = pcl_transforms(
        bbox_pos_img,
        bbox_size_img,
        K,
        focal_at_image_plane,
    ...
    # Vector manipulation to use torch.bmm and transform original 2D pose
    # to virtual camera coordinates
    ...
    PCL_canonical_2d_pose = PCL_canonical_2d_pose.squeeze(-1).view(
        batch_size, num_joints, -1)

    # Convert from homogeneous coordinate by dividing x and y by z
    virt_2d_pose = torch.div(PCL_canonical_2d_pose[:, :, :-1],
                             PCL_canonical_2d_pose[:, :, -1].unsqueeze(-1))
...
def virtual_camera_rotation_from_position(position):
    ...
    R_virt2orig = torch.stack([
        d1x, -x * y * d1xy1x, x * d1xy, 0 * x, n1x * d1xy, y * d1xy, -x * d1x,
        -y * d1xy1x, 1 * d1xy
    ],
                              dim=1).reshape([-1, 3, 3])
    return R_virt2orig
...
def bmm_homo(K_inv, bbox_center_img):
    ...
    ones = torch.ones([batch_size, 1],
                      dtype=torch.float).to(bbox_center_img.device)
    bbox_center_px_homo = torch.cat([bbox_center_img, ones],
                                    dim=1).reshape([batch_size, 3, 1])
