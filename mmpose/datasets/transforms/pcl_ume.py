...
import numpy as np
from nreal_data_tool.utils import affine, camera

from .pcl import make_look_at_matrix
...
def gen_crop_parameters_from_points(
    ...
    * focal_multiplier: when less than 1, we are zooming out a little. The effect on the image
    ...
    new_world_to_eye = make_look_at_matrix(orig_world_to_eye_xf, crop_center,
                                           camera_angle)
    if mirror_img_x:
