...
from mmpose.codecs.utils.post_processing import get_simcc_3d_maximum
...
class SimCCLabel3D(BaseKeypointCodec):
    ...
    def encode(self,
        ...
        if self.smoothing_type == 'gaussian':
            x_labels, y_labels, z_labels, keypoint_weights = \
                self._generate_gaussian(keypoints, keypoints_visible)
    ...
    def decode(self, simcc_x: np.ndarray, simcc_y: np.ndarray,
        ...
        keypoints, scores = get_simcc_3d_maximum(simcc_x, simcc_y, simcc_z)
        ...
        if self.use_dark:
            ...
            keypoints[:, :, 1] = refine_simcc_dark(keypoints[:, :, 1], simcc_y,
                                                   y_blur)
            keypoints[:, :, 2] = refine_simcc_dark(keypoints[:, :, 2], simcc_z,
                                                   z_blur)

        keypoints /= self.simcc_split_ratio
        keypoints[..., 2] = (keypoints[..., 2] / self.input_size[2] -
                             0.5) * self.depth_bound
    ...
    def _generate_gaussian(
        ...
        for n, k in product(range(N), range(K)):
            ...
            if left >= W or top >= H or right < 0 or \
                    bottom < 0 or near >= D or far < 0:
                keypoint_weights[n, k] = 0
                continue
