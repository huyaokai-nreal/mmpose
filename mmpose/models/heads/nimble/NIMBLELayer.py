...
def procrustes_align(
    ...
    outer_prod = torch.matmul(torch.transpose(from_centered, 1, 2), to_centered).to(dtype=torch.float32)
...
class NIMBLELayer(torch.nn.Module):
    ...
    def __init__(self, base_path, device, shape_ncomp=20, pose_ncomp=30, tex_ncomp=10, use_pose_pca=False, nimble_mano_vreg=None):
        ...
        self.register_buffer("th_verts", pm_dict['vert'].squeeze())   # shape (14970, 3)
        self.register_buffer("th_faces", pm_dict['face'].squeeze())    # shape (27106, 3)
