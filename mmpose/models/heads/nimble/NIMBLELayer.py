...
class NIMBLELayer(torch.nn.Module):
    ...
    def __init__(self,
        ...
        pm_dict_name = f'{base_path}/NIMBLE_DICT_9137.pkl'
        tex_dict_name = f'{base_path}/NIMBLE_TEX_DICT.pkl'
        ...
        self.bone_v_sep = pm_dict['bone_v_sep']  # 3345
        self.skin_v_sep = pm_dict['skin_v_sep']  # -5990

        self.register_buffer('th_verts',
                             pm_dict['vert'].squeeze())  # shape (14970, 3)
        self.register_buffer('th_faces',
                             pm_dict['face'].squeeze())  # shape (27106, 3)
        ...
        self.register_buffer('tex_diffuse_basis',
                             tex_dict['diffuse']['basis'].squeeze())
        self.register_buffer('tex_diffuse_mean',
                             tex_dict['diffuse']['mean'].squeeze())
        self.register_buffer('tex_diffuse_std',
                             tex_dict['diffuse']['std'].squeeze())
        self.register_buffer('tex_normal_basis',
                             tex_dict['normal']['basis'].squeeze())
        self.register_buffer('tex_normal_mean',
                             tex_dict['normal']['mean'].squeeze())
        self.register_buffer('tex_normal_std',
                             tex_dict['normal']['std'].squeeze())
        self.register_buffer('tex_spec_basis',
                             tex_dict['spec']['basis'].squeeze())
        self.register_buffer('tex_spec_mean',
                             tex_dict['spec']['mean'].squeeze())
        ...
        self.skin_v_surface_mask = pm_dict['skin_v_surface_mask'].type(
            torch.bool)
        self.skin_v_node_weight = dis_to_weight(pm_dict['skin_v_gd'], 30, 50)

        if nimble_mano_vreg is not None:
            self.register_buffer('nimble_mano_vreg_fidx',
                                 nimble_mano_vreg['lmk_faces_idx'])
            self.register_buffer('nimble_mano_vreg_bc',
                                 nimble_mano_vreg['lmk_bary_coords'])
        else:
    ...
    def nimble_to_mano(self, verts, is_surface=False):
        ...
        nimble_mano = torch.cat([
            vertices2landmarks(skin_v, skin_f.squeeze(),
                               self.nimble_mano_vreg_fidx[i],
                               self.nimble_mano_vreg_bc[i]).unsqueeze(0)
            for i in range(20)
        ])
        nimble_mano_v = nimble_mano.mean(0)
        return nimble_mano_v

    def compute_warp(self, batch_size, points, skinning_weights,
                     full_trans_mat):
    ...
    def generate_hand_shape(self, betas, normalized=True):
        ...
        jreg_bone_joints = torch.matmul(self.jreg_bone,
                                        th_v_shaped[:, :self.bone_v_sep])
    ...
    def generate_texture(self, alpha, normalized=True):
        ...
        if normalized:
            alpha_real_d = alpha * self.tex_diffuse_std[:self.
                                                        tex_ncomp].reshape(
                                                            1, -1)
            alpha_real_n = alpha * self.tex_normal_std[:self.
                                                       tex_ncomp].reshape(
                                                           1, -1)
            alpha_real_s = alpha * self.tex_spec_std[:self.tex_ncomp].reshape(
                1, -1)

        x_d = (self.tex_diffuse_basis[:, :self.tex_ncomp] @ alpha_real_d.T
               ).T + self.tex_diffuse_mean.unsqueeze(0).repeat(batch_size, 1)
        x_d = x_d.reshape(batch_size, self.tex_size, self.tex_size, 3)

        x_n = (self.tex_normal_basis[:, :self.tex_ncomp] @ alpha_real_n.T
               ).T + self.tex_normal_mean.unsqueeze(0).repeat(batch_size, 1)
        x_n = x_n.reshape(batch_size, self.tex_size, self.tex_size, 3)

        x_s = (self.tex_spec_basis[:, :self.tex_ncomp] @ alpha_real_s.T
               ).T + self.tex_spec_mean.unsqueeze(0).repeat(batch_size, 1)
    ...
    def forward(self, pose_param, shape_param):
        ...
        mesh_v, bone_joints = self.forward_full(th_v_shaped, full_pose, None,
                                                jreg_joints, self.sw, self.pbs)
    ...
    def forward_full(self,
        ...
        if global_scale is not None:
            ...
            verts_scale = global_scale.expand(th_verts.shape[0],
                                              th_verts.shape[1])
    ...
    def mesh_collision(self, floating_verts, floating_verts_normals,
                       steady_verts, steady_faces):
        ### go to trimesh
        batch_size = floating_verts.shape[0]
        for i in range(batch_size):
            mesh_muscle = trimesh.Trimesh(
                steady_verts[i].detach().cpu().numpy(),
                steady_faces.squeeze().detach().cpu().numpy())
            skin_in_muscle = mesh_muscle.contains(
                floating_verts[i].detach().cpu().numpy())
            skin_surf_in_muscle = self.skin_v_surface_mask & torch.from_numpy(
                skin_in_muscle).to(self.device)
            if skin_surf_in_muscle.sum() <= 1:
                continue
            inside_verts = floating_verts[i][skin_surf_in_muscle].reshape(
                -1, 3)
            inside_verts_normal = floating_verts_normals[i][
                skin_surf_in_muscle].reshape(-1, 3)

            ## moving target using ray-triangle hit
            locations, index_ray, index_tri = mesh_muscle.ray.intersects_location(
                inside_verts.squeeze().detach().cpu().numpy(),
                inside_verts_normal.squeeze().detach().cpu().numpy())
            locations = locations + 2 * inside_verts_normal.squeeze().detach(
            ).cpu().numpy()[index_ray]  # outside 2 mm
            index_ray = torch.from_numpy(index_ray).to(self.device)
            offset = torch.zeros_like(inside_verts)
            offset[index_ray] = torch.from_numpy(locations).float().to(
                self.device) - inside_verts[index_ray]
            ...
            soft_result = floating_verts[i] + (
                self.skin_v_node_weight.unsqueeze(-1) * skin_v_offset).sum(1)
