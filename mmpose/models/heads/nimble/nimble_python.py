...
kintree_parents = np.array([
    -1, 0, 1, 2, 3, 0, 5, 6, 7, 8, 0, 10, 11, 12, 13, 0, 15, 16, 17, 18, 0, 20,
    21, 22, 23
])

landmark_index = [
    1143, 3788, 567, 2963, 342, 2382, 2269, 3108, 3444, 3139, 114, 2514, 123,
    2380, 4430, 1184, 1859, 3754, 880, 3817, 1906
]
identity_rot = torch.tensor(np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]]))
kp_index = [
...
def forward_3d(shape_param, local_pose_matrix):
    ...
    jreg_joints = scale_factor.view(
        scale_factor.shape[0], 1, 1) * jreg_joints_relative + root_jreg_joints

    # 得到旋转后的角度
    local_pose_matrix = torch.tensor(local_pose_matrix).unsqueeze(0)
    th_pose_map, th_rot_map = th_posemap_axisang_2output_usematrix(
        local_pose_matrix)
    root_rot = torch.eye(3,
                         3).unsqueeze(0).repeat(batch_size, 1,
                                                1).to(local_pose_matrix.device)
    th_results = []
    ...
    for i in range(STATIC_JOINT_NUM - 1):
        ...
        joint_j = jreg_joints[:, i_val_joint, :].contiguous().view(
            batch_size, 3, 1)
        parent = kintree_parents[i_val_joint]
        parent_j = jreg_joints[:,
                               parent, :].contiguous().view(batch_size, 3, 1)
        joint_rel_transform = th_with_zeros(
            torch.cat([joint_rot.to(joint_j.device), joint_j - parent_j], 2))
    ...
    for i in range(STATIC_JOINT_NUM):
        padd_zero = torch.zeros(
            1, dtype=jreg_joints.dtype, device=jreg_joints.device)
        joint_j = torch.cat(
            [jreg_joints[:, i],
    ...
    pm_dict = np.load(
        '/data/stliu/NIMBLE_model/assets/NIMBLE_DICT_9137.pkl',
        allow_pickle=True)
    skin_v_index = pm_dict['skin_v_sep']
    skinning_weight = pm_dict['all_sw'].to(
        jreg_joints.device).double()[skin_v_index:]
    points_pose_bs = pm_dict['all_pbs'].to(
        jreg_joints.device).double()[skin_v_index:]
    ...
    th_verts = compute_warp(batch_size, points_pose_bs, skinning_weight,
                            th_results2)
    return th_jtr, th_verts


def compute_warp(batch_size, points, skinning_weights, full_trans_mat):
...
if __name__ == '__main__':
    ...
    local_pose_matrix = np.array([[
        -0.2078, -0.3727, 0.9044, -0.9669, -0.0617, -0.2475, 0.1480, -0.9259,
        -0.3475
    ],
                                  [
                                      0.2026, -0.9699, 0.1348, 0.5783, 0.0075,
                                      -0.8158, 0.7903, 0.2432, 0.5625
                                  ],
                                  [
                                      0.7759, -0.2654, 0.5724, 0.0148, 0.9146,
                                      0.4041, -0.6307, -0.3050, 0.7135
                                  ],
                                  [
                                      0.9998, -0.0068, -0.0199, 0.0057, 0.9985,
                                      -0.0549, 0.0203, 0.0548, 0.9983
                                  ],
                                  [
                                      -0.8260, -0.1810, 0.5338, 0.0473,
                                      -0.9660, -0.2543, 0.5616, -0.1848, 0.8065
                                  ],
                                  [
                                      0.2340, -0.8936, -0.3829, 0.9323, 0.3180,
                                      -0.1724, 0.2759, -0.3167, 0.9075
                                  ],
                                  [
                                      -0.2748, -0.8339, -0.4787, 0.7709,
                                      0.1065, -0.6280, 0.5747, -0.5416, 0.6135
                                  ],
                                  [
                                      0.9816, -0.1860, -0.0432, 0.1865, 0.9824,
                                      0.0091, 0.0408, -0.0169, 0.9990
                                  ],
                                  [
                                      0.7137, 0.3943, 0.5790, -0.6843, 0.5692,
                                      0.4558, -0.1498, -0.7215, 0.6760
                                  ],
                                  [
                                      -0.7356, -0.4106, -0.5388, 0.6361,
                                      -0.1450, -0.7579, 0.2331, -0.9002, 0.3678
                                  ],
                                  [
                                      -0.8636, -0.1702, -0.4746, 0.5031,
                                      -0.2292, -0.8333, 0.0331, -0.9584, 0.2836
                                  ],
                                  [
                                      0.9439, -0.3179, -0.0890, 0.3226, 0.9455,
                                      0.0437, 0.0702, -0.0700, 0.9951
                                  ],
                                  [
                                      -0.9846, 0.1715, -0.0346, -0.1559,
                                      -0.9500, -0.2704, -0.0793, -0.2608,
                                      0.9621
                                  ],
                                  [
                                      0.1308, 0.7591, 0.6377, -0.8935, 0.3689,
                                      -0.2559, -0.4296, -0.5363, 0.7265
                                  ],
                                  [
                                      0.6455, 0.6738, 0.3596, -0.6922, 0.7151,
                                      -0.0976, -0.3229, -0.1859, 0.9280
                                  ],
                                  [
                                      -0.1974, 0.9733, -0.1169, -0.8709,
                                      -0.2289, -0.4349, -0.4501, 0.0160, 0.8929
                                  ],
                                  [
                                      -0.7808, 0.4320, -0.4513, 0.1935,
                                      -0.5196, -0.8322, -0.5940, -0.7371,
                                      0.3221
                                  ],
                                  [
                                      0.8361, 0.5443, 0.0683, -0.5388, 0.8383,
                                      -0.0835, -0.1027, 0.0330, 0.9942
                                  ],
                                  [
                                      -0.6152, 0.7716, 0.1618, -0.1281, 0.1046,
                                      -0.9862, -0.7779, -0.6274, 0.0345
                                  ]])
    ...
    nlayer = NIMBLELayer(
        '/data/stliu/NIMBLE_model/assets',
        'cpu',
        use_pose_pca=False,
        pose_ncomp=30)
