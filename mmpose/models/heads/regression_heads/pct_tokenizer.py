...
class PCT_Tokenizer(nn.Module):
    """Tokenizer of Pose Compositional Tokens. paper ref: Zigang Geng et al.
    ...
    def __init__(self, stage_pct, tokenizer=None, num_joints=20):
        super().__init__()
    ...
    def forward(self, joints, joints_feature, cls_logits, train=True):
        ...
        if train and self.stage_pct == 'tokenizer':
            ...
            sync_encodings, sync_dw = torch.split(combined,
                                                  [n_encodings, n_dw])
            ...
            part_token_feat = encode_feat + (part_token_feat -
                                             encode_feat).detach()
        else:
            e_latent_loss = None
        ...
        part_token_feat = self.decoder_token_mlp(part_token_feat).transpose(
            2, 1)
    ...
    def init_weights_self(self, pretrained=''):
