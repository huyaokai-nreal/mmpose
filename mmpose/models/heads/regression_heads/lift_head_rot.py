...
class LiftNimbleHead(BaseModule):
    ...
    def __init__(
        ...
        self.used_nimble_para = [
            51, 52, 54, 57, 39, 40, 42, 45, 27, 28, 30, 33, 15, 16, 18, 21, 0,
        ...
        if use_sim_nimble:
            self.nimble_layer = sim_NIMBLELayer(
                ...
                reg_shape_type=self.reg_shape_type)
        else:
    ...
    def initial_skeleton(self, ):
        ...
        _, reset_bone_joints = self.nimble_layer(
            rot_init, shape_init, init_shape_pose=True)
        reset_pose = self.nimble_layer.return_pose(rot_init)
    ...
    def postprocess(self, output, left_hand, leftcam_xy):
        ...
        if self.use_svd:
            shape_vector = output[:, pose_len:(pose_len + self.shape_ncomp), 0,
                                  0]
        ...
        if self.use_nimble_part_para:
            ...
            rot_vector_zeros = torch.zeros(
                (rot_vector.shape[0], rot_vector.shape[1]),
                device=rot_vector.device,
                dtype=torch.float32)
            rot_vector_zeros[:, used_nimble_para] = \
                rot_vector[:, used_nimble_para].to(torch.float32)
        ...
        if not self.use_svd:
            ...
            special_rotation = torch.tensor(
                [[-0.95179426, 0.27416083, 0.13756282],
                 [-0.27311822, -0.55333527, -0.78690947],
                 [-0.13962139, -0.78654683, 0.60153965]])
            Rot[mask] = special_rotation.unsqueeze(0)
            rebuild_joints_with_scale = torch.bmm(
                Rot.cuda(),
                rebuild_joints_with_scale.permute(0, 2, 1)).permute(0, 2, 1)
    ...
    def decode_svd(
        ...
        rigid_points = pred_pts_features.reshape(pred_pts_features.shape[0],
                                                 -1, 3)
    ...
    def trans_3d_2_2d(self, hand3d_point, leftcam_cam_matrix,
                      rightcam_cam_matrix, lr_rot_matrix, lr_p):
    ...
    def loss(self,
        ...
        hand3d_pred, hand3d_nimble_xyz, hand3d_rebuild_xyz, trans_xyz = \
            self.postprocess(output, left_hand, leftcam_xy)
        ...
        if self.use_nimble_pca:
            ...
            gt_nimble_pose = self.nimble_layer.convert_rot_to_pca(
                gt_nimble_pose_rot)
        else:
            pre_nimble_pose = output[:, :self.pose_ncomp, 0, 0]
            pre_nimble_trans = trans_xyz
            ...
            if self.use_nimble_part_para:
                used_nimble_para = torch.tensor(self.used_nimble_para)
                rot_vector_zeros = torch.zeros(
                    (pre_nimble_pose.shape[0], pre_nimble_pose.shape[1]),
                    device=pre_nimble_pose.device,
                    dtype=torch.float32)
                rot_vector_zeros[:, used_nimble_para] = \
                    pre_nimble_pose[:, used_nimble_para].to(torch.float32)
        ...
        # 2d重投影损失 这里把pre设置为gt
        leftcam_uv_pre, rightcam_uv_pre = self.trans_3d_2_2d(
            hand3d_pred, leftcam_cam_matrix, rightcam_cam_matrix,
            lr_rot_matrix, lr_p)
        leftcam_uv_gt, rightcam_uv_gt = self.trans_3d_2_2d(
            hand3d_gt, leftcam_cam_matrix, rightcam_cam_matrix, lr_rot_matrix,
            lr_p)

        # xyz比例约束
        proportion_xyz_pre = self.cal_proportion(leftcam_uv_pre,
                                                 leftcam_cam_matrix)
        proportion_xyz_gt = self.cal_proportion(leftcam_uv_gt,
                                                leftcam_cam_matrix)
