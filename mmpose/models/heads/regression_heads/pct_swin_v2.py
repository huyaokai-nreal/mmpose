...
from mmpose.models.heads.nimble.ckpt_load import load_checkpoint_swin
    ...
    def __init__(self,
        ...
        if self.relative_coords_table_type != 'none':
            ...
            if relative_coords_table_type == 'linear':
                ...
                print(f'norm8_log_bylayer: \
                        [{self.window_size}] ==> [{pretrain_window_size}]')
...
class ConvPatchMerging(nn.Module):
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm, postnorm=True):
        ...
        self.reduction = nn.Conv2d(
            dim, 2 * dim, kernel_size=3, stride=2, padding=1)
    ...
    def __init__(self,
        ...
        if downsample is not None:
            self.downsample = downsample(
                dim=dim, norm_layer=norm_layer, postnorm=postnorm)
        else:
...
class SwinV2TransformerRPE2FC(nn.Module):
    ...
    def self_init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            ...
            load_checkpoint_swin(
                self,
                pretrained,
                strict=False,
                map_location='cpu',
                rpe_interpolation=self.rpe_interpolation)
