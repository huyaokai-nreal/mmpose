...
class LiftHeadStandardSpherical(BaseModule):
    ...
    def postprocess(self, output, norm_leftcam_xyz, norm_rightcam_xyz,
        ...
        leftcam_Z_scale = output[:, :21].view(B, K, 1) / norm_leftcam_xyz[:, :,
                                                                          2:]
        leftcam_XYZ = (norm_leftcam_xyz * leftcam_Z_scale).view(B * K, 3, 1)
        ...
        rightcam_XYZ = (norm_rightcam_xyz * rightcam_Z_scale).view(B * K, 3, 1)
    ...
    def standardize_stereo(self, leftcam_xy, rightcam_xy, leftcam_to_virtual_R,
                           rightcam_to_virtual_R):
        standard_leftcam_xyz = self.align_monocular_to_parallel_stereo(
            leftcam_xy, leftcam_to_virtual_R)
        standard_rightcam_xyz = self.align_monocular_to_parallel_stereo(
            rightcam_xy, rightcam_to_virtual_R)
        norm_leftcam_xyz = F.normalize(
            standard_leftcam_xyz.clone(), p=2, dim=-1).clone()
        norm_rightcam_xyz = F.normalize(
            standard_rightcam_xyz.clone(), p=2, dim=-1).clone()
    ...
    def loss(self,
        ...
        hand3d_pred, leftcam_XYZ, rightcam_XYZ = self.postprocess(
            output, norm_leftcam_xyz, norm_rightcam_xyz, leftcam_to_virtual_R,
