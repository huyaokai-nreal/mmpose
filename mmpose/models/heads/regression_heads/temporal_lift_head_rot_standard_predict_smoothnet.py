...
class TemporalLiftNimbleHeadStandardPredictSmoothNet(LiftNimbleHeadStandard):
    ...
    def __init__(self,
        ...
        self.rle_loss_func = RLELoss(
            dim=3, flow_model_pretrain_path=flow_model_pretrain)
    ...
    def smoothnet_predict_forward(self,
        ...
        for i in range(seq_len):
            root_info_history = torch.zeros(
                (B, 21, self.smooth_window_len)).cuda()
            local_info_history = torch.zeros(
                (B, self.pose_ncomp, self.smooth_window_len)).cuda()
    ...
    def predict(self,
        ...
        raw_feats = output[:, :, :, 0, 0].clone()
        output = self.smoothnet_predict_forward(output, feats_history, 1)
    ...
    def loss(self,
        ...
        B = output.shape[0]
        history_output = torch.zeros((B, output.shape[2], self.seq_len))
        output = self.smoothnet_predict_forward(output, history_output,
                                                self.seq_len)
        ...
        (loss_root_predict, loss_local_curent, loss_pinch, loss_nimble_trans,
         loss_root_smooth, loss_local_smooth) = losses
