...
class LiftHead_Rotation(BaseModule):
    ...
    def postprocess(self, output, left_hand, leftcam_xy):
        ...
        shape_vector = output[:,63:83, 0, 0]
        if self.use_svd:
            pre_pt_features = output[:,83:, 0, 0]
        
        if not self.use_svd:
            skin_v, bone_joints = self.nimble_layer.forward(rot_vector, shape_vector, handle_collision=False)
            ...
            init_root_rot = torch.zeros_like(output[:, :3, 0, 0])
            new_rot_vector = torch.cat((init_root_rot, rot_vector[:,3:]),dim=1)
            skin_v, bone_joints = self.nimble_layer.forward(new_rot_vector, shape_vector, handle_collision=False)
            rebuild_joints = bone_joints[:, self.kp_index, :]
            root_rebuild_joints = rebuild_joints[:, 0:1, :]
            
            
            
            # print(f"############# new_rot_vector:{new_rot_vector[0]}    shape_vector:{shape_vector[0]}     rebuild_joints:{rebuild_joints[0]}     ")
            # rot_t1 = new_rot_vector[0]
            # shape_t1 = shape_vector[0]
            # skin_v_t1, bone_joints_t1 = self.nimble_layer.forward(rot_t1.unsqueeze(0), shape_t1.unsqueeze(0), handle_collision=False)
            # root_rebuild_joints_t1 = bone_joints_t1[:, self.kp_index, :]
            # print(f"$$$$$$$$$$$  root_rebuild_joints_t1:{root_rebuild_joints_t1}")
            
        #     from IPython import embed; embed()
        #     new_rot_vector = torch.tensor(np.array([ 0.0000,  0.0000,  0.0000,  0.2472,  0.1205, -0.8813,  0.0546, -0.3677,
        #  0.4380, -0.2559, -0.4690, -0.1035, -0.5796,  0.9688, -0.0206,  0.0685,
        #  0.0507, -0.3401,  0.0950, -0.3044, -0.0837,  0.4778, -0.4465,  0.3962,
        # -0.0903, -0.4829, -0.0016, -0.1165, -0.1981, -0.0778, -0.3770,  0.2993,
        # -0.1573, -0.1929, -0.0217,  0.4275,  0.1284, -1.1162, -0.5879,  0.2754,
        #  0.2913, -0.0914, -0.7007, -0.1213, -0.7788,  0.4971,  0.0363,  0.5781,
        # -0.0954,  0.1592,  0.3225, -0.3843,  0.3806,  0.4048,  0.2395, -0.3975,
        # -0.2048, -1.1631, -0.3125,  0.5884])).cuda().to(torch.float32)
        #     shape_vector = torch.tensor(np.array([ -0.0769,  0.2253, -0.4675, -0.7461,  0.4060,  0.6729,  0.1166,  0.3896,
        # -0.0556, -0.6445, -0.4326,  0.0748, -0.3525,  0.2255, -0.1151, -0.6318,
        # -0.5386,  0.3975,  0.3730, -0.6611])).cuda().to(torch.float32)
        #     skin_v, bone_joints = self.nimble_layer.forward(new_rot_vector.unsqueeze(0), shape_vector.unsqueeze(0), handle_collision=False)
        #     rebuild_joints = bone_joints[:, self.kp_index, :]
    ...
    def decode_svd(self,
        ...
        wrist_xfs = procrustes_align(from_points, rigid_points)
        return wrist_xfs
    ...
    def predict(self,
        ...
        leftcam_uv_gt, rightcam_uv_gt = self.trans_3d_2_2d(hand3d_pred, leftcam_cam_matrix, rightcam_cam_matrix, lr_rot_matrix, lr_p)
        proportion_xyz_gt = self.cal_proportion(leftcam_uv_gt,leftcam_cam_matrix)
        nimble_Z = hand3d_pred[:,:,2:].view((output.shape[0], 21, 1))
        rebuild_xyz = torch.cat((proportion_xyz_gt * nimble_Z, nimble_Z), dim=2).view((output.shape[0], 21, 3))
        ...
        return rebuild_xyz, uv_coord_im_pred_global_distort
    ...
    def loss(self,
        ...
        # debug问题用的
        # gt_nimble_shape = nimble_info["nimble_shape"]
        # gt_nimble_pose = nimble_info["nimble_pose"].reshape(-1,60)
        # gt_nimble_trans = nimble_info["nimble_trans"]
        # new_output = torch.cat((gt_nimble_pose, gt_nimble_trans, gt_nimble_shape), dim=1).unsqueeze(-1).unsqueeze(-1)
        # gt1, gt2, gt3 = self.postprocess(new_output, left_hand, leftcam_xy)
        # import pdb; pdb.set_trace()
        # from IPython import embed; embed()
    
    
        # 直接监督rot和trans
        pre_nimble_pose = output[:, :60, 0, 0]
        ...
        pred_for_loss = [
            ...
            rightcam_uv_pre, dist_pred, proportion_xyz_pre, pre_nimble_pose[:,3:], pre_nimble_trans
        ]
        targ_for_loss = [
            hand3d_gt, hand3d_gt, hand3d_gt, leftcam_uv_gt, rightcam_uv_gt,
            dist_gt, proportion_xyz_gt, gt_nimble_pose[:,3:], gt_nimble_trans
        ]
