...
from .ume_head import create_multi_view_fusion_layers, apply_ftl_to_feature_maps
from .ume_head_seq import SimpleConvRNN
from mmpose.models.utils.gmlp_v2 import gMLPForPose

class PointToFeatNet(nn.Module):
    def __init__(self, a=6, b=8):
        super(PointToFeatNet, self).__init__()
        self.a, self.b = a,b
        self.fc = nn.Linear(21 * 2, b * b)
        self.conv = nn.Conv2d(1, a, kernel_size=3, padding=1)

    def forward(self, x):
        batch_size = x.size(0)
        x = x.reshape(batch_size, -1)
        x = self.fc(x)
        x = x.view(batch_size, 1, self.b, self.b)
        x = self.conv(x)
        return x

@MODELS.register_module()
class TemporalUmeLiftNimbleHeadStandardE2e(LiftNimbleHeadStandard):
    ...
    def __init__(self,
        ...
        self.last_layer = nn.Sequential(
            ...
            nn.Linear(feat_dim, self.output_num))
        self.feat_channel = feat_channel
        self.proj_layer1 = PointToFeatNet(a=feat_channel, b=feature_map_shape[0])
        self.proj_layer2 = PointToFeatNet(a=feat_channel, b=feature_map_shape[0])
    ...
    def forward(self, feats, cam_xf, mem, seq_len) -> dict:
        ...
        feats1 = self.proj_layer1(feats[...,:2])   # left
        feats2 = self.proj_layer2(feats[...,2:])   # right
        feats1 = feats1.view((B, seq_len) + feats1.shape[-3:])
        feats2 = feats2.view((B, seq_len) + feats2.shape[-3:])

        feats = torch.stack((feats1, feats2), dim=1)
        cam_xf = cam_xf.view((B, seq_len) + cam_xf.shape[-3:])
        outputs = torch.zeros(
            (B, seq_len, self.output_num)).to(feats.device)
        sigmas = torch.zeros((B, seq_len, 21, 3)).to(feats.device)
        for i in range(seq_len):
            feat = feats[:, :, i].view((-1, self.feat_channel) + self.feature_map_shape)
            _cam_xf = cam_xf[:, i, :, :, :].clone()
            # fuse multiv feat
            _cur_fused_features = self.forward_feature_fuse(
                feat, _cam_xf)
            # temporal module
            temporal_features, mem = self.temporal.forward_tem_feat(
            ...
            temporal_features = temporal_features.view(B, -1)
    ...
    def preprocess(self, feats, batch_data_samples, mode):
        ...
        cam_xf = torch.eye(4).repeat(feats.shape[0], 2, 1, 1).to(feats.device)
        # cam_xf[:,1,0,-1] = batch_data_samples[0].meta['virtual_baseline']
        return {
            'feats': feats,
    ...
    def predict(self,
        ...
        output, all_sigmas, mems = self.forward(
            data['feats'], data['cam_xf'], mems, seq_len=1)
    ...
    def loss(self,
        ...
        valid_mask = data['valid_mask'] == 1
        output, all_sigmas, _ = self.forward(
            data['feats'], data['cam_xf'], None, seq_len=self.seq_len)
    ...
    def forward_feature_fuse(self, feats, cam_xf):
        extrinsics_xf = torch.inverse(cam_xf.view(-1, 4, 4)).view(
            cam_xf.shape)  # B,2,4,4  world_to_camera
    ...
    def compute_multiv_features(
        ...
        ) = self._compute_multiv_xfs(extrinsics_xf)
        ...
        return multiv_fused_img_features

    def _compute_multiv_xfs(self,
                            extrinsics_xf: torch.Tensor
        ...
        xf_0 = extrinsics_xf[:, 0:1].clone() # left world_to_camera
        xf_inv = torch.inverse(extrinsics_xf.view(-1, 4, 4)).reshape(
            extrinsics_xf.shape)   # camera_to_world
        xf_to_world = xf_inv
        scaled_to_canonical_xf = xf_0 @ xf_to_world
        return scaled_to_canonical_xf