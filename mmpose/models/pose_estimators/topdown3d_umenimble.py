...
class TopdownPoseUmeNimbleEstimator(BaseModel):
    ...
    def loss(self, inputs: Tensor, data_samples: SampleList) -> dict:
        ...
        """
        feats = self.extract_feat(inputs)[0]
    ...
    def predict(self, inputs: Tensor, data_samples: SampleList) -> SampleList:
        ...
        feats = self.extract_feat(inputs)[0]
        hand3d_pred, hand2d_pred, sigmas = self.head.predict(
            feats, data_samples, test_cfg=self.test_cfg)

        batch_pred_instances = []
        for b in range(hand3d_pred.shape[0]):
            batch_pred_instances.append(
                InstanceData(
                    keypoints3d=hand3d_pred[b:b + 1, ...],
                    keypoints3d_scores=sigmas[b:b + 1],
                    keypoints=hand2d_pred[b:b + 1, ...],
    ...
    def add_pred_to_datasample(self, batch_pred_instances: InstanceList,
        ...
        batch_data_samples = batch_data_samples[::2]
        ...
        for pred_instances, pred_fields, data_sample in zip_longest(
            ...
            if data_sample.meta['flipped'] and not data_sample.meta['ume']:
...
@MODELS.register_module()
class TopdownPoseUmeNimbleEstimatorSeq(TopdownPoseUmeNimbleEstimator):
    ...
    def predict(self, inputs: Tensor, data_samples: SampleList) -> SampleList:
        ...
        for b in range(self.seq_len):
            sub_feats = feats[:, 2 * b:2 * b + 2, :]
            sub_feats = sub_feats.reshape((sub_feats.shape[0] *
                                           sub_feats.shape[1], ) +
                                          feats.shape[2:])
            data_sample_id_list = [[
            ...
            hand3d_pred, hand2d_pred, mem, sigma = self.head.predict(
                sub_feats, [data_samples[i] for i in data_sample_id_list],
                mem,
