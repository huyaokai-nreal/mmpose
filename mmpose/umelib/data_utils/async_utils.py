...
do correct error handling and cleanup. It's very easy to silently
drop errors in asyncio, and very easy to cause hangs.
...
async code. :class:`aclosing` is required for deterministic cleanup of async
...
def run_coroutine_threadsafe(coro_or_task: Awaitable[T],
                             loop: asyncio.AbstractEventLoop
                             ) -> concurrent.futures.Future:
    """Like :func:`asyncio.run_coroutine_threadsafe` but cancellation works
    properly.

    The problem:

    :meth:`concurrent.futures.Future.cancel` sets the future to a "done" state
    *immediately*. It provides no way whatsoever to wait for cleanup
    to complete, nor to report errors that occurred during cleanup.

    :meth:`asyncio.run_coroutine_threadsafe` returns a Future so it inherits
    this problem. This leads to race conditions in cleanup code and makes
    it nearly impossible to robustly shut things down.

    This function returns a derived `Future` that overrides `cancel()` to
    request the task be cancelled, but doesn't mark the Future as done
    until the task has actually finished cancellation.
...
class _AFuture(concurrent.futures.Future):
    ...
    def __init__(self, coro_or_task, loop):
        if not (asyncio.iscoroutine(coro_or_task)
                or isinstance(coro_or_task, asyncio.Task)):
...
class TaskGroup:
    ...
    complete by the time the group context exits. If the context exits
    normally, it waits for all the tasks to complete. If it exits via
    an exception, it cancels all tasks and then waits for them.
    ...
    On normal exit, it will wait for all tasks to complete. If any of
    them raise an error, the remaining tasks will be cancelled.
    ...
    There's an excellent explanation of this model at
    ...
    if sys.version_info >= (3, 8):
        ...
        def create_task(self, awaitable, name=None) -> asyncio.Task:
            ...
            if self.cancelled:
                raise ValueError(
                    'attempting to create task in cancelled taskgroup')
    ...
    async def __aexit__(self, exc_type, exc, tb):
        if exc_type is not None:
            if self._cancel_on_error or isinstance(exc,
...
class AsyncToSyncChannel(Generic[T]):
    ...
    However, there is no bounded queue type for sending data between
    async tasks and threads. This class provides that for the
    ...
    def close(self, discard=False):
        ...
        Attempting to iterate a closed queue will continue to yield any
        remaining items (unless discard=True) and then raise `StopIteration`.
