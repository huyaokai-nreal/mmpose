...
"""This module defines methods for running asyncio code from non-asyncio
code."""
...
def get_event_loop():
    ...
    if th is None:
        ...
        pass
        # logger.info(
        #     f"recreating event loop thread for \
        #         {os.getpid()} after fork from {th.original_pid}"
        # )
...
class _AsyncToSyncIterator:
    ...
    def send(self, val):
        try:
            return run(self._iter.ascend(val))
        except StopAsyncIteration as e:
            raise StopIteration from e
...
def async_to_sync_iterator(async_iter):
    """Turn an async iterator into a normal iterator via
    `nested_async.run`:func:
...
def prefetch_sequence(seq: Iterable[Awaitable[T]],
                      prefetch_size: int) -> Iterable[T]:
