...
from typing import (AsyncIterable, Awaitable, Dict, Iterable, Iterator, List,
                    Mapping, Optional, Sequence, Sized, Tuple, TypeVar, Union)
...
from mmpose.umelib.data_utils import fs, nested_async
# from .idxbinfile import OBJECT_DTYPE, RawField, TorchIdx
from .idxbinfile import TorchIdx
from .split import Split

# from multiprocessing.dummy import Pool
# from typing import (Any, AsyncIterable, Awaitable, Dict, Iterable, Iterator,
# List, Mapping, Optional, Sequence, Sized, Tuple, Type,
# TypeVar, Union)
...
def find_torchdata_folders(root: str,
                           log_print: bool = True) -> Dict[str, List[str]]:
    ...
    if fs.basename(root) == 'synthetic':
        split_names.remove('testing')
...
def filter_torchdata_folders(
    ...
    required_filenames = {
        f + ext
        for f in required_fields for ext in extensions
    }
    ...
    for (folder, found) in folders.items():
        ...
        if missing:
            logger.debug(f'Skipping torchdata folder {folder} \
                because some dataset files were missing: {missing}')
        else:
...
class InMemoryTorchBin(AsyncDataset[np.ndarray]):
    ...
    def __init__(self, idx_file: TorchIdx) -> None:
        ...
        self._bin_data = shared_memory.SharedMemory(
            create=True, size=len(bin_buffer))
...
class AsyncTorchBin(AsyncDataset[np.ndarray], AsyncIterable[np.ndarray]):
    ...
    def __repr__(self):
        return f'SingleIdxBinDataset(len={len(self)}, \
    path={[self.idx_file.source]})'
    ...
    async def __anext__(self):
        ...
        async for x in dataset:     sum += np.sum(x)
        """
...
def get_field_dataset(
    ...
    field
        The specific field name to load (i.e. for `spam.torch.
        idx`/`spam.torch.bin`, the field name is `spam`).
...
class SingleFolderAsyncDataset(AsyncDataset[Dict[str, np.ndarray]]):
        ...
        for field, field_dataset, field_n in field_dataset_iterator:
            if field_dataset is not None:
                if self.n is not None and self.n != field_n:
                    raise ValueError(
                        f'folder has mismatched field sizes: {field}')
                self.n = field_n
                if field_dataset:
                    self._fields[field] = field_dataset
            else:
                raise ValueError(
                    f'required field {field} not found in folder {folder}, \
                        which only has {fs.listdir(folder)}')
    ...
    async def __getitem__(self, idx: int):
        dd = {name: await data[idx] for (name, data) in self._fields.items()}
        # mono:[16,2,250,250]
        # labels:dict_keys(['extrinsics',
        #                   'intrinsics',
        #                   'enclosing_points',
        #                   'hand',
        #                   'hand_model',
    ...
    def cacheable_size_bytes(self) -> int:
        """Return the total size in bytes this dataset might occupy in
        memory."""
        n = 0
        for f in self._fields.values():
...
def find_dataset(
    ...
    *,
    ...
    folders = {}
    for root in roots:
        data_folders = find_torchdata_folders(root, log_print)  # dict
        if log_print:
            logger.info(f'Found {len(data_folders)} data folders in {root}')
    ...
    catted = {}
    if len(roots) == 1 and log_print:
        logger.info(f'Under {roots[0]}:')
    for s in splits:
        ds = per_folder[s.value]  # list of training or testing
        if len(ds) == 0:
            continue
        ...
        catted[s] = torch.utils.data.ConcatDataset(ds)  # concat dataset
        if log_print:
            logger.info(f'{s}: found total of {len(catted[s])} frames, \
                    resident size up to {sz/1e9:.3g}GB')
    return catted


def subsample(dataset: Dataset[T],
              *,
              portion: float = 1.0,
              max_len: Optional[int] = None) -> Dataset[T]:
...
class Sampler(torch.utils.data.Sampler[int]):
    ...
    ==========
    dataset
        Dataset to sample. This is only needed for
    calling `len()`, so any sized
    ...
    distrib_info
        ...
        Otherwise you may run into weird behaviors during training.
    """
    ...
    def _get_distrib_info(self, required: bool):
        if self._distrib_info is None:
            # env = os.getenv() if required else _env
            env = os.getenv()
            if env:
...
class AsyncToIterableDataset(IterableDataset[T]):
    ...
    sampler
        Iterable over sample indices. If provided,
    it must be specifically a
        :class:`distrib.Sampler`, not an arbitrary sampler.
        An arbitrary sampler would not work correctly if the loader
        has `num_workers > 1`, because torch assumes there is only
        one sampler per gpu process, not one per io worker.
    ...
    max_prefetch
        The maximum number of items to prefetch per io_worker.
        This prefetching is the main purpose of the whole
        `Dataset[Awaitable]` system: for storage with high bandwidth
        but high latency, you may need a large number of reads in
        flight to cover latency.

        Setting a large number here should be harmless,
        as the prefetcher will ramp up to avoid sending out more
        reads than necessary, except that if your items are very
        large it may consume a lot of memory.
    ...
    sampler
        ...
        `set_epoch()` on it to set the random number seed.
        (Note that the epoch is automatically incremented on each
        iteration though the sampler, so `set_epoch` is only needed
        at initialization.)
    """
    ...
    def __init__(
        ...
        if sampler is None:
            ...
            raise ValueError(
                'either sampler or shuffle should be specified, not both')
        self.dataset = dataset
    ...
    def __iter__(self):
        ...
        with closing(
                nested_async.prefetch_sequence(samples,
                                               self.max_prefetch)) as seq:
            yield from seq
