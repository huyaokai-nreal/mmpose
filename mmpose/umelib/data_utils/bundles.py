...
def is_dictlike(obj: Any) -> bool:
    ...
    return (dataclasses.is_dataclass(obj) or hasattr(obj, '_asdict')
...
def asdict(obj):
    ...
    NamedTuple, or dataclass. If it's an actual Mapping it will be returned as-
    is.
    """
...
def asdict_rec(obj):
    ...
    if dataclasses.is_dataclass(obj):
        return {
            f.name: asdict_rec(getattr(obj, f.name))
...
def to_cuda(obj):
    """Call `t.to(device)` for all tensors in obj."""
    return map_fields(lambda t: t.cuda(), obj, only_type=torch.Tensor)
...
def collate(batch, device=None):
    ...
    batch
        ...
        dicts, tuples, namedtuples, and dataclasses will be recursively
        collated per-field. Numpy arrays and torch tensors will be stacked
        along a new outermost dimension. Any other types are simply
    ...
    def stack_fn(t_list):
        ...
        raise TypeError(
            f"Can't stack tensors: unknown type {type(value0)} found")
...
def group(batch, group_fn):
    ...
    This function is more generic than collate: collate will stack the tensor
    fields whereas this function also enables arbitrary grouping functions
    (i.e. torch.cat).
    """
    ty = type(batch[0])
    value0 = batch[0]
    ...
    if isinstance(value0, tuple):
        return tuple(
            group([v[i] for v in batch], group_fn) for i in range(len(value0)))
