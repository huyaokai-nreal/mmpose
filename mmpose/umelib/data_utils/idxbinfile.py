...
def _get_bin_path_for_idx(idx_path: str):
    """if idx file path ends with ".torch.idx", then replace it with
    ".torch.bin" to get bin file path otherwise, replace the full extension in
    idx file path with ".torch.bin"."""
...
class TorchIdx:
    ...
    byte_offsets : np.ndarray
        `byte_offsets[i:i+1]` gives the span containing
    frame `i` in the .torch.bin file
    """
    ...
    def __init__(self,
        ...
        if data[1] == 0:
            ...
            raise ValueError(
                f'unsupported version {data[1]} in .torch.idx file {path}')
        ...
        if dtype != OBJECT_DTYPE and dtype.itemsize != self.itemsize:
            raise ValueError(f'item size {self.itemsize} not compatible with '
        ...
        dims = [
            tuple(sizes[dim_offsets[i]:dim_offsets[i + 1]]) for i in range(N)
        ]
        ...
        self.shape = None
        self.byte_offsets_array: Optional[
            np.ndarray] = data_offsets * self.itemsize
        if self.dtype != OBJECT_DTYPE and all(
                len(d) == len(dims[0]) for d in dims):
            ...
            if (dims == dims[0]).all():
                ...
                if data_offsets[0] == 0:
                    ...
                    # store the byte offsets; instead just recompute
                    # them on the fly using the known fixed size.
    ...
    def __repr__(self):
        if self.is_uniform:
            return f'TorchIdx(shape={self.shape}, \
                dtype={self.dtype}, source={self.source})'

        else:
            return f'TorchIdx(N={len(self)}, dtype={self.dtype}, \
                is_uniform=False, source={self.source})'
    ...
    def byte_offsets(self, start: int, end: int) -> np.ndarray:
        ...
        return (math.prod(shape[1:]) * self.itemsize *
    ...
    def view_buffer(self, buffer: Buffer) -> Union[np.ndarray, ISeq[RawField]]:
        ...
        if shape is None:
            return map_dataset(
                self.view_buffer_at, range(len(self)), buffer=buffer)
        else:
            # uniform shape: return a single big tensor. Note that
            # this still just creates a view on the given buffer;
            # if it is a mmapped file, this will not cause it to be
            # materialized in memory until the data is actually accessed.
            return np.ndarray(shape=shape, dtype=self.dtype, buffer=buffer)

    def view_buffer_at(self, index: int, buffer: Buffer) -> RawField:
        """Given a buffer over the whole .bin file, return view of just one
        frame.
    ...
    def view_frame(self, index: int, buffer: Buffer) -> RawField:
        """Given a buffer over a single frame's data, return view of that
        frame.
        ...
        if self._bin_format == BinFormat.MSGPACK:
            ...
            return np.ndarray(
                shape=self.dims[index], dtype=self.dtype, buffer=buffer)
    ...
    def _check_buf_size(self, istart, istop, buffer):
        ...
        if binsz != bufsz:
            raise ValueError(f'expected {binsz} bytes but got {bufsz} for \
                    {self.bin_path}[{istart}:{istop}]')
    ...
    def item_shape(self, i: Optional[int] = None) -> Tuple[int, ...]:
        ...
        If `i` is `None` (the default), then all items must have the same
        shape, and that shape is returned.
        """
        if i is None:
            s = self.shape
            if s is None:
                raise ValueError(
                    f'Dataset does not have uniform shape: {self.source}')
            return s[1:]
    ...
    def _idx_buf_to_int64(self, buffer: Buffer) -> np.ndarray:
        ...
        if buffer.ndim != 1:
            raise ValueError(f'.torch.idx data has invalid shape: \
                    {buffer.shape}; require ndim=1')
        if buffer.format == 'B':
            # byte or uint8 data
            if len(buffer) % 8 != 0:
                raise ValueError(
            ...
            raise ValueError(
                f".torch.idx data has invalid format {buffer.format}: \
                    expected 'B' (bytes)  or 'q' or 'l' (int64)")
...
_np_to_idx_type: Dict[Tuple[str, BinFormat],
                      int] = {s: n
                              for n, s in _idx_to_np_type.items()}
