...
from lib.common.hand import NUM_JOINTS_PER_HAND, RIGHT_HAND_INDEX, HandModel
from lib.common.hand_skinning import skin_landmarks

from .tracking_result import SingleHandPose


def neutral_joint_angles(up: HandModel,
                         lower_factor: float = 0.5) -> torch.Tensor:
...
def landmarks_from_hand_pose(hand_model: HandModel, hand_pose: SingleHandPose,
                             hand_idx: int) -> np.ndarray:
    """Compute 3D landmarks in the world space given the hand model and hand
    pose."""
...
def _get_crop_points_from_hand_pose(
    ...
    crop_points = []
    crop_points.append(
        landmarks_from_hand_pose(hand_model, gt_hand_pose, hand_idx))
