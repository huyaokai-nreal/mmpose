...
import numpy as np
import torch
from lib.common.hand import NUM_HANDS, HandModel, scaled_hand_model
from lib.data_utils import bundles
from lib.models.regressor import RegressorOutput
from lib.models.umetrack_model import (InputFrameData, InputFrameDesc,
                                       InputSkeletonData)
...
class HandTracker:
    ...
    def gen_crop_cameras(
        ...
        for hand_idx, per_hand_crop_cameras in crop_cameras.items():
            if not per_hand_crop_cameras or len(
                    per_hand_crop_cameras) < min_num_crops:
    ...
    def track_frame(self,
                    sample: InputFrame,
                    hand_model: HandModel,
                    crop_cameras: Dict[int,
                                       Dict[int,
                                            camera.PinholePlaneCameraModel]],
                    frame_idx=-1) -> TrackingResult:
        if not crop_cameras:
            # Frame without hands
            self.reset_history()
        ...
        # print(frame_desc.sample_range)
        # print(frame_desc.hand_idx)
        with torch.no_grad():
            regressor_output = bundles.to_device(
                self._model.regress_pose_use_skeleton(frame_data, frame_desc,
                                                      skeleton_data),
                torch.device('cpu'),
            )
    ...
    def track_frame_and_calibrate_scale(
        ...
        if not crop_cameras:
            ...
            self.reset_history()
            return TrackingResult()
        frame_data, frame_desc, _ = self._make_inputs(sample, None,
                                                      crop_cameras)
    ...
    def _make_inputs(
        ...
        for hand_idx, crop_camera_info in crop_cameras.items():
            ...
            for cam_idx, crop_camera in crop_camera_info.items():
                ...
                crop_image = _warp_image(view_data.camera, crop_camera,
                                         view_data.image)
            ...
            if image_idx > sample_range_start:
                ...
                sample_range_n_hands.append(
                    np.array([sample_range_start, image_idx]))
        ...
        return bundles.to_device((frame_data, frame_desc, skeleton_data),
                                 self._device)
