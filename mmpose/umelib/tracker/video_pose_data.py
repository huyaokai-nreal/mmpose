...
class VideoStream:
    ...
    def __iter__(self) -> Iterator[np.ndarray]:
        ...
        stream = container.streams.video[0]
        print(
            f'Opened ({int(stream.average_rate)} fps) video from {self._data_path}'
        )
...
class SyncedImagePoseStream:
    ...
    def __iter__(self):
        for frame_idx, raw_mono in enumerate(self._image_stream):
            # print(self._hand_pose_labels.wrist_transforms)
            # assert 1==2
            gt_tracking = {}
            for hand_idx in range(0, 2):
                if self._hand_pose_labels.hand_confidences[frame_idx,
                                                           hand_idx] > 0:
                    gt_tracking[hand_idx] = SingleHandPose(
                        ...
                        wrist_xform=self._hand_pose_labels.wrist_transforms[
                            frame_idx, hand_idx],
                        hand_confidence=self._hand_pose_labels.
                        hand_confidences[frame_idx, hand_idx],
                    )
            multi_view_images = raw_mono.reshape(
                raw_mono.shape[0], len(self._hand_pose_labels.cameras), -1)

            invalid_camera_to_world = (
                self._hand_pose_labels.camera_to_world_transforms[frame_idx].
                sum() == 0)
            ...
            for cam_idx in range(0, len(self._hand_pose_labels.cameras)):
                cur_camera = self._hand_pose_labels.cameras[cam_idx].copy(
                    camera_to_world_xf=self._hand_pose_labels.
                    camera_to_world_transforms[frame_idx, cam_idx], )
                views.append(
                    ViewData(
                        image=multi_view_images[:, cam_idx, :],
                        camera=cur_camera,
                        camera_angle=self._hand_pose_labels.
                        camera_angles[cam_idx],
                    ))
