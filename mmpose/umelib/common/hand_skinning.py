...
def _finger_fk(joint_local_xfs: torch.Tensor,
               parent_transform: torch.Tensor) -> List[torch.Tensor]:
    """
    ...
    for i in range(4):
        transform_mats.append(
            torch.matmul(transform_mats[-1], joint_local_xfs[:, i]))
    return transform_mats[2:]


def _joint_local_transform(rotation_axis: torch.Tensor,
    ...
    local_transform = torch.eye(
        4, dtype=angle_axis.dtype, device=angle_axis.device)
    local_transform = local_transform.unsqueeze(
        dim=0).repeat(angle_axis.shape[0], 1, 1)

    rot_mat = so3_exp_map(angle_axis)
    translation = rest_pose_flat - torch.matmul(
        rot_mat, rest_pose_flat.unsqueeze(dim=-1)).squeeze(dim=-1)
...
def _lbs(trans_mats: torch.Tensor,
         skinned_points: torch.Tensor) -> torch.Tensor:
    """
    Input:
        trans_mats: (B, 17, 4, 4)
    ...
    fk_points = torch.matmul(trans_mats,
                             skinned_points).sum(dim=2).squeeze(dim=-1)
    return fk_points


def _get_skinning_weights(bone_indices: torch.Tensor,
    ...
    '''Offset all the bones linearly from 0 to (bs*n_lms*n_frames)
    so that we can directly
    index into the flattened weight matrix and set the
    corresponding skinning weights
    '''
    flat_idx_offset = torch.arange(
        0, bs * n_lms, device=bone_indices.device) * n_frames
    bone_flat_idx = bone_indices.long() + flat_idx_offset.reshape(bs, n_lms, 1)
    skin_mat = torch.zeros(
        bs * n_lms * n_frames,
        device=bone_weights.device,
        dtype=bone_weights.dtype)
...
def _skin_points(
    ...
    numel = torch.flatten(
        joint_angles, end_dim=-2).shape[0] if len(leading_dims) else 1
