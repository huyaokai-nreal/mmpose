...
class CameraProjection(Protocol):
    ...
    def unproject(cls, p):
        ...
        `project(unproject(p)) == p` `unproject(project(v)) == v / |v|`
        """
        ...
...
class Fisheye62CameraModel(NamedTuple):
    ...
    def evaluate(self: Sequence[float], p: np.ndarray) -> np.ndarray:
        ...
        r10 = r4 * r6
        r12 = r6 * r6
        radial = (1 + k1 * r2 + k2 * r4 + k3 * r6 + k4 * r8 + k5 * r10 +
                  k6 * r12)
        uv = p * radial
...
class CameraModel(CameraProjection, abc.ABC):
    ...
    def __init__(
        ...
        # (see issue 4790 on pylint github)
        self.width = width
        self.height = height
    ...
    def world_to_eye(self, v):
        ...
        return affine.transform_vec3(self.camera_to_world_xf.T,
                                     v - self.camera_to_world_xf[:3, 3])

    def eye_to_world(self, v):
        """Apply inverse camera camera_to_world_xf to eye points `v` to get
        world coords."""
    ...
    def window_to_eye(self, w):
        """Unproject 2d window coordinates to unit-length 3D eye
        coordinates."""
    ...
    def crop(
        ...
        return type(self)(
            ...
            self.distort,
            self.camera_to_world_xf
            if camera_to_world_xf is None else camera_to_world_xf,
        )
...
class PinholePlaneCameraModel(PerspectiveProjection, CameraModel):
    ...
    def uv_to_window_matrix(self):
        ...
        return np.array([[self.f[0], 0, self.c[0]], [0, self.f[1], self.c[1]],
                         [0, 0, 1]])
...
def read_camera_from_json(js):
    ...
    for name in distort_params:
        if name in js.keys():
            coeffs.append(js[name])
        else:
            coeffs.append(0)
    # print(coeffs)
