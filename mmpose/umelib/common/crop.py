...
def gen_crop_parameters_from_points(camera_orig: camera.CameraModel,
                                    pts_world,
    ...
    * pts_world: points in the world space that must be projected
    inside the image by the generated world to eye transform and intrinsics.
    ...
    * mirror_img_x: whether to flip the image. A typical use case is we
    ...
    left hand data only
    * camera_angle: how the camera is oriented physically so that we can
    rotate the object of interest to the 'upright' direction
    * focal_multiplier: when less than 1, we are zooming out a little.
    ...
    if train and random.random() < 0.5:
        ...
        # noise = np.random.rand(1,3)*0.006-0.003

    if train and random.random() < 0.5:
        camera_angle = np.random.rand() * 360
        # print(focal_multiplier,noise,camera_angle)
    # focal_multiplier = 1.1
    ...
    # crop_center = crop_center + noise
    # print(crop_center)
    # focal_multiplier = 1.4
    # print(np.random.randn(1,3))
    new_world_to_eye = affine.make_look_at_matrix(orig_world_to_eye_xf,
                                                  crop_center, camera_angle)
    if mirror_img_x:
