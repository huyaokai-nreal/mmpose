...
def _PCK_curve(errors: np.ndarray, mask: np.ndarray,
...
def normalized_AUC(x: np.ndarray,
                   y: np.ndarray,
                   y_max: float = 1.0) -> np.ndarray:
    """Given curves sharing the same x-axis, computes normalized AUC for each
    of the curves.
    ...
    auc = ((x[1:] - x[:-1]).reshape(1, -1) *
           ((y[..., 1:] + y[..., :-1]) * 0.5)).sum(axis=-1)
