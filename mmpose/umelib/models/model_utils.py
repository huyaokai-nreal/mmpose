...
def procrustes_align(
    ...
    outer_prod = torch.matmul(
        torch.transpose(from_centered, 1, 2), to_centered)
    ...
    xfs[:, 0:3, 3] = (
        to_mean - torch.matmul(xfs[:, 0:3, 0:3].clone(),
                               from_mean.unsqueeze(-1)).squeeze())
...
def create_backbone(
    ...
    start_layers = nn.Sequential(
        ...
        nn.BatchNorm2d(start_planes, momentum=0.1),
        nn.ReLU(),
        nn.MaxPool2d(kernel_size=2, stride=2),
    )
    resnet_input_size = (int(input_size[0] / 2), int(input_size[1] / 2))

    resnet_model = resnet.create_model(arch, start_layers, resnet_input_size,
                                       start_planes)
    ...
    proj_layer = nn.Conv2d(
        n_img_maps, n_out_channels, kernel_size=1, padding=0)
...
def create_multi_view_fusion_layers_t(
    nc_in: int,
    nc_out: int,
    n_blocks: int,
) -> nn.Module:
...
def compute_singlev_xfs(intrinsics: torch.Tensor,
                        canonical_focal_length: float = 200) -> torch.Tensor:
    """The image backbone is expected to generate 3D point features. Since
    predicting 3D points without intrinsics is infeasible, the code here
    factorizes the.

    intrinsics into 2 parts: K = K_canonical * S. The 3D point features from the
    image itself should correspond to K_cannionical so that we need to apply S to the
    ...
    singlev_scaled_to_orig_xf = (
        torch.eye(4, dtype=intrinsics.dtype,
                  device=intrinsics.device).unsqueeze(0).repeat(n_elem, 1, 1))
...
def create_pose_regression_layers(
    ...
    for _ in range(n_blocks):
        pose_regression_layers.append(
            resnet.BasicBlock(n_in_channels, n_in_channels))
    pose_regression_layers.append(
