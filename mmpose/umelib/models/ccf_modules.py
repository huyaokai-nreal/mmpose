...
class CrossCueFusion(nn.Module):
    ...
    def __init__(self, cv_hypo_num=72, mid_dim=72):
        super().__init__()
        ...
        self.mono_expand = nn.Sequential(
            nn.Conv2d(
                ...
                kernel_size=1,
                padding=0,
                stride=1), nn.BatchNorm2d(self.mid_dim), nn.ReLU(inplace=True),
            nn.Conv2d(
                self.cv_hypo_num,
                self.mid_dim,
                kernel_size=1,
        ...
        if self.residual_connection:
            self.mono_reg = nn.Sequential(
                nn.Conv2d(
                    self.cv_hypo_num, self.mid_dim, kernel_size=1, padding=0),
                nn.BatchNorm2d(self.mid_dim), nn.ReLU(inplace=True))
    ...
    def forward(self, mono_pseudo_cost, cost_volume):
        ...
        multi_q = self.lin_multi_q(multi_feat).view(b, -1,
                                                    h * w).permute(0, 2, 1)
        ...
        # mono_out = torch.bmm(mono_v, mono_atten.permute(0,2,1))
        ...
        # multi_out = torch.bmm(multi_v, multi_atten.permute(0,2,1))
...
class MultiGuideMono(nn.Module):
    ...
    def __init__(self, cv_hypo_num=32, mid_dim=32, input_size=(256, 512)):
        ...
        if not self.is_reduce:
            self.mono_expand = nn.Sequential(
                nn.Conv2d(
                    ...
                    kernel_size=3,
                    padding=1,
                    stride=2), nn.BatchNorm2d(self.mid_dim),
        ...
        if self.residual_connection:
            self.mono_reg = nn.Sequential(
                nn.Conv2d(
                    self.cv_hypo_num, self.mid_dim, kernel_size=3, padding=1),
                nn.BatchNorm2d(self.mid_dim), nn.ReLU(inplace=True),
                nn.Conv2d(
                    self.cv_hypo_num, self.mid_dim, kernel_size=3, padding=1),
                nn.BatchNorm2d(self.mid_dim), nn.ReLU(inplace=True))
