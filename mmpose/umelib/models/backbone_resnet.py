...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.bn1 = nn.BatchNorm2d(num_features=planes, momentum=0.1)
        self.relu = nn.ReLU()
        ...
        self.bn2 = nn.BatchNorm2d(num_features=planes, momentum=0.1)
        self.downsample = downsample
        self.stride = stride

    def forward(self, x):
        residual = x
        # print(x.cpu().detach().numpy().var(0).var(1).var(1))
        # assert 1==2
        out = self.conv1(x)
        out = self.bn1(out)
...
class ResNetBase(nn.Module):
    ...
    def __init__(
        ...
        if strides is None:
            strides = [1, 2, 2, 2]
        assert (len(layers) == len(layers_in_planes)
    ...
    def _make_layer(
        ...
        if stride != 1 or in_planes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                nn.BatchNorm2d(
                    num_features=planes * block.expansion, momentum=0.1),
            )

        layers = []
        layers.append(
            block(in_planes, planes, stride, downsample, groups=groups))
        for _i in range(1, blocks):
            layers.append(
                block(planes * block.expansion, planes, groups=groups))
