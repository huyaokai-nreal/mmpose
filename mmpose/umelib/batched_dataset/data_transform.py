...
import mmpose.umelib.common.camera as camera
from mmpose.umelib.common import crop
from mmpose.umelib.common.hand import HandModel, mirrored_hand_model
from mmpose.umelib.data_utils import bundles
from .sample import RawSample, parse_raw_buffers
...
def _compute_resample_matrix(
    ...
    Return `resample_xf`: taking a point in `camera_new` to a point in
    `camera_orig`
    """
...
def _resample_images_batched(images_orig, images_new, resample_xfs):
    """`resample_r`, `resample_t`: from pixel `(u,v,1)` in img new to pixel in
    img orig."""
    ...
    mul_res = np.tensordot(
        resample_r, grid, axes=([2], [2])).transpose(0, 2, 3, 1)
    # add resample_t
    grid_xfed = mul_res.reshape(n_images, -1, 3) + np.expand_dims(
        resample_t, axis=1)
    grid_xfed = grid_xfed.reshape(mul_res.shape)
    # compute homogeneous coords by dividing `x, y` by `z`
    homo_coords = grid_xfed[:, :, :, 0:2] / grid_xfed[:, :, :, 2:]
    # boolean flag indicating whether transformed coords are in orig image
    coord_mask = ((homo_coords[:, :, :, 0] >= 0)
    ...
    grid_int = np.repeat(
        grid.reshape(-1, *grid.shape)[:, :, :, 0:2], n_images, axis=0)
    ...
    images_new[(img_idx, grid_int[:, 1],
                grid_int[:, 0])] = (fx_00 * (x1 - x) * (y1 - y) + fx_10 *
                                    (x - x0) * (y1 - y) + fx_01 * (x1 - x) *
                                    (y - y0) + fx_11 * (x - x0) *
                                    (y - y0)) / ((x1 - x0) * (y1 - y0))


def _gen_crop_matrices(orig_extrinsics: np.ndarray,
    ...
    corresponding crop matrices extrinsics_xf, new_intrinsics, and resample_xf.

        Args:
        - `orig_extrinsics`: of shape `(#views, 4, 4)` (world_to_eye
        transformation)
        ...
        - `focal_multiplier`: multiplier of the focal length to make hands
        fit inside a crop

        Return (tuple of following)
        - `extrinsics_xf`: world_to_eye transformation for new camera used to
        generate the resampled images 2d pts in new image wrt their center to
    take up all space of new image.
        See `crop.gen_crop_parameters_from_points()`.
        - `new_intrinsics`: `(#frames, #views, 3, 3)`,
        intrinsics of new camera.
    ...
    #     camera_angle = np.random.rand()*360
    for view_idx in range(n_views):
        ...
        resample_xf[view_idx] = _compute_resample_matrix(
            camera_orig, camera_new)

    return (extrinsics_xf, new_intrinsics, resample_xf)


def _perspective_crop_images(orig_images: np.ndarray,
                             orig_extrinsics: np.ndarray,
                             orig_intrinsics: np.ndarray,
                             crop_points: np.ndarray,
                             hand_idx: int,
                             crop_size: Tuple[int, int],
                             train=False) -> List[np.ndarray]:
    """Given a sequence data, for each frame, generate a new camera looking
    through the hand crop center, return a new image that encloses all given
    points `crop_points`. Returned images have intensities normalized to [0,
    1].
    ...
    Return (tuple of following)
    - `output_images`: `(#frames, #views, crop_size, crop_size)`,
    pixel values are normalized to 0~1.
    - `extrinsics_r`, `extrinsics_t`: T_camera_world for new camera
    used to generate the resampled images 2d pts in new image wrt
    their center to take up all space of new image.
    See `crop.gen_crop_parameters_from_points()`.
    ...
    '''for each frame, generate a hand crop region and its
    corresponding new camera
    '''
    ...
    # if train and random.random()<0.5:
    #     focal_multiplier = np.random.rand()*0.19 + 0.855
    ...
    cropped_left_images = np.zeros([n_frames, n_views, *crop_size],
                                   dtype=scalar_type)
    _resample_images_batched(
        *bundles.map_fields(lambda t: t.reshape(-1, *t.shape[2:]), (
            orig_images, cropped_left_images, resample_matrices)))
    if train and random.random() < 0.5:
        noise = np.random.rand(16, 1, 1, 1) * 145 + 25
        cropped_left_images = cropped_left_images + noise
        cropped_left_images = np.clip(cropped_left_images, 0, 255)
...
def prepare_inputs_targets(sample: RawSample,
                           crop_size: Tuple[int, int],
                           train=False) -> Tuple[ModelInput, ModelTarget]:
    ...
    left_generic_hand_model = mirrored_hand_model(generic_hand_model,
                                                  to_th(sample.hand) == 1)
    ...
    # crop_pts = crop_pts + noise
    # if train:
    #     noise = np.random.rand(1,3)*0.002-0.001
    #     crop_pts = crop_pts + noise
    # print(crop_pts)
...
def preprocess(data: Dict[str, Any],
               crop_size: Tuple[int, int],
               train=False) -> Tuple[ModelInput, ModelTarget]:
    """Transform each sequence into model input & groundtruth.

    Args:
    - `data`: Dict with keys "mono", "msgpack_pose_data",
    "msgpack_s_solved_data"
    """
