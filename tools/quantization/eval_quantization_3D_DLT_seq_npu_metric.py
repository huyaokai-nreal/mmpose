...
def main():
    ...
    for dataset_idx, dataset_path in enumerate(dataset_list):
        ...
        while idx < len(dataset):
            item = dataset[idx]['data_samples']
            
            img_id = str(item.metainfo['img_id']).zfill(8)
            anno_id = str(item.id).zfill(8)
            # if img_id != "00000218":
            #     idx += 1
            #     continue
            ...
            feat_x = np.loadtxt(os.path.join(args.result_dir, f'{dataset_name}_{img_id}_{anno_id}/feat_x.txt'), dtype=np.float32)
            feat_y = np.loadtxt(os.path.join(args.result_dir, f'{dataset_name}_{img_id}_{anno_id}/feat_y.txt'), dtype=np.float32)
            feat_x = feat_x.reshape(1, 21, input_size*2)
            feat_y = feat_y.reshape(1, 21, input_size*2)
            # for 3d
            angle = np.loadtxt(os.path.join(args.result_dir, f'{dataset_name}_{img_id}_{anno_id}/angle.txt'), dtype=np.float32)
            svd = np.loadtxt(os.path.join(args.result_dir, f'{dataset_name}_{img_id}_{anno_id}/svd_pt.txt'), dtype=np.float32)
            score = np.loadtxt(os.path.join(args.result_dir, f'{dataset_name}_{img_id}_{anno_id}/score.txt'), dtype=np.float32)
            sigma = np.loadtxt(os.path.join(args.result_dir, f'{dataset_name}_{img_id}_{anno_id}/sigma.txt'), dtype=np.float32)
            ...
            uv_coord = uv_coord * np.array([input_size, input_size])
            ...
            hand_type = "left_hand" if item.meta['category_id'] == 1 else "right_hand"
            left_hand = [1 if item.meta['category_id'] == 1 else 0]
            
            vritual_camera = item.meta['virtual_camera']
            ...
            preds = [
                InstanceData(
                    ...
                    keypoints=item.gt_instances.keypoints,
                    keypoint_scores=np.ones((1, 21)),
                )]
            
            data_sample = dataset[idx]['data_samples']
        ...
        print(f"{dataset_name}, valid_num: {valid_num}")
