...
def main():
    ...
    result_files = [os.path.splitext(file)[0] for file in list(os.listdir(args.result_dir))]
    ...
    for idx, dataset_path in enumerate(dataset_list):
        ...
        while idx < len(dataset):
            ...
            valid_num += 1
            feat_x = np.loadtxt(os.path.join(args.result_dir, f'{dataset_name}_{img_id}.png', 'feat_x.txt'), dtype=np.float32)
            feat_y = np.loadtxt(os.path.join(args.result_dir, f'{dataset_name}_{img_id}.png', 'feat_y.txt'), dtype=np.float32)
            feat_z = np.loadtxt(os.path.join(args.result_dir, f'{dataset_name}_{img_id}.png', 'feat_z.txt'), dtype=np.float32)
            
            feat_x = feat_x.reshape(1, 21, input_size*2)
            feat_y = feat_y.reshape(1, 21, input_size*2)
            feat_z = feat_z.reshape(1, 21, input_size*2)
